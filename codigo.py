...
# CONFIGURAÇÕES DA ESTAÇÃO REECHARGE (Foco 100% Comercial)
POTENCIA_NOMINAL_VAGA = 7.0  # kW por carregador
LIMITE_POTENCIA_HUB = 15.0  # Limite total que a estação aguenta (Demanda)
TARIFA_BASE_COMERCIAL = 0.80 # Tarifa única comercial baseada na resolução ANEEL


def simular_log_ocpp(mensagem_tipo, payload):
    """Simula o envio consistente de frames no protocolo OCPP 1.6J via WebSocket"""
    frame_ocpp = [2, f"msg_{random.randint(1000, 9999)}", mensagem_tipo, payload]
    print(f"\n[OCPP >> CENTRAL] {json.dumps(frame_ocpp, indent=2)}")
    time.sleep(0.4)


def calcular_tarifa_dinamica(hora_atual):
    """Aplica tarifação dinâmica com base na cobrança comercial e horários de pico"""
    tarifa = TARIFA_BASE_COMERCIAL
...
def gerenciar_potencia_hub(vagas_ativas):
    """Controle inteligente de demanda limitando a potência por veículo (Smart Charging)"""
...
def simular_agente_ia_preditiva(vagas_ativas):
    """
    Núcleo de IA Preditiva e NLP: Analisa o cenário atual e gera insights
    sobre a operação e sugestões de janelas ociosas.
    """
    print("\n" + "*" * 50)
    print(" 🤖 AGENTE IA CHARGEGRID - ANÁLISE EM TEMPO REAL")
    print("*" * 50)
    time.sleep(1.5) # Simula o tempo de processamento dos algoritmos

    qtd = len(vagas_ativas)

    # Processamento de Linguagem Natural (NLP) traduzindo dados para o gestor
    if qtd >= 3:
        print(" 💡 INSIGHT OPERACIONAL: A infraestrutura está operando próxima à capacidade máxima.")
        print(" ⚠️ ALERTA PREDITIVO: Alta probabilidade de ativação do Smart Charging nas próximas horas.")
    elif qtd > 0:
        print(" 💡 INSIGHT OPERACIONAL: Rede operando com margem segura. Distribuição de potência equilibrada.")
    else:
        print(" 💡 INSIGHT OPERACIONAL: Estação totalmente ociosa no momento.")

    # Sugestão de janela de carregamento otimizada
    print(" 📊 SUGESTÃO DO MOTOR DE IA: Direcione campanhas de recarga para a janela entre 23h e 05h para maximizar a eficiência energética e evitar tarifas de pico.")
    print("*" * 50)


def executar_simulador():
    vagas = {}  # Gerencia sessões ativas (Vaga: Dados do Veículo)
    historico_geral = []
    ...
    while True:
        print("\n" + "=" * 45)
        print("    CENTRAL DE CONTROLE COMERCIAL REECHARGE   ")
        print("=" * 45)
        print(f" Vagas Ocupadas: {len(vagas)}/4 | Limite da Estação: {LIMITE_POTENCIA_HUB} kW")
        print("-" * 45)
        print("[1] Conectar Veículo (Nova Sessão Comercial)")
        print("[2] Exibir Monitoramento & Análise de IA")
        ...
        if opcao == "1":
            ...
            print(f"\n--- Iniciando Cadastro Comercial para a Vaga #{vaga_id} ---")
            ...
            # Montando a estrutura da sessão ativa (Categoria exclusivamente Comercial)
            vagas[vaga_id] = {
            ...
            print(f"\n✅ Veículo conectado com sucesso na Vaga #{vaga_id} (Perfil Comercial)!")
        ...
        # OPERAÇÃO 2: MONITORAMENTO E INTELIGÊNCIA ARTIFICIAL
        elif opcao == "2":
            if not vagas:
                print("\nNenhum veículo conectado no momento.")
                simular_agente_ia_preditiva(vagas)
                continue
            ...
            for vid, dados in vagas.items():
                tarifa_atual, status_t = calcular_tarifa_dinamica(dados["hora_inicio"])
                print(
                    f" Vaga #{vid} | Perfil: {dados['categoria']} ({status_t}) | Tarifa: R${tarifa_atual:.2f}/kWh | Carga: {dados['soc']}% ({dados['energia_recarregada']:.2f}kWh)")
            print("·" * 50)

            # Aciona o Agente IA
            simular_agente_ia_preditiva(vagas)

        # OPERAÇÃO 3: SIMULAR CICLO DE RECARGA
        elif opcao == "3":
            if not vagas:
            ...
            while concluidos < len(vagas):
                ...
                for vid, dados in vagas.items():
                    if dados["soc"] < 100:
                        incremento_energia = (potencia_por_vaga * 0.1)
        ...
        # OPERAÇÃO 4: LIBERAR VAGA E GERAR COBRANÇA
            ...
            if vaga_fechar in vagas:
                ...
                tarifa, _ = calcular_tarifa_dinamica(dados["hora_inicio"])
                ...
                print("\n" + "=" * 40)
                print("       REECHARGE - RECIBO COMERCIAL       ")
                print("=" * 40)
                ...
                del vagas[vaga_fechar]
            else:
                print("Vaga inválida ou sem veículo ativo.")

        # OPERAÇÃO 5: ENCERRAMENTO E RELATÓRIO GLOBAL
        elif opcao == "5":
            if vagas:
            ...
            print("\n\n" + "=" * 60)
            print("         RELATÓRIO GERENCIAL DE RECARGAS      ")
            print("=" * 60)
            if not historico_geral:
                ...
                for reg in historico_geral:
                    print(
                        f"Sessão #{reg['id']} | Vaga {reg['vaga']} | Comercial | Carga: {reg['energia']:.2f} kWh | Recebido: R$ {reg['total']:.2f}")
